classdef CamSideUav  < CamUav
  ...
  methods
    ...
    function gamma = gamma(self)
      gamma(1,1) = self.q(1,1) + self.q(5,1)*cot(self.q(7,1))*sin(self.q(3,1));
      gamma(2,1) = self.q(2,1) - self.q(5,1)*cot(self.q(7,1))*cos(self.q(3,1));
    ...
    function u = feedBackLin(self,v)
      ...
      A(1,1) = - q4*(self.vel*sin(q3) - q6*cos(q3)*cot(q7) + q5*q8*cos(q3)*(cot(q7)^2 + 1) + q4*q5*cot(q7)*sin(q3)) - q8*(q6*sin(q3)*(cot(q7)^2 + 1) + q4*q5*cos(q3)*(cot(q7)^2 + 1) - 2*q5*q8*cot(q7)*sin(q3)*(cot(q7)^2 + 1)) - q6*(q8*sin(q3)*(cot(q7)^2 + 1) - q4*cos(q3)*cot(q7));

      A(2,1) =  q4*(self.vel*cos(q3) + q6*cot(q7)*sin(q3) - q5*q8*sin(q3)*(cot(q7)^2 + 1) + q4*q5*cos(q3)*cot(q7)) - q8*(q4*q5*sin(q3)*(cot(q7)^2 + 1) - q6*cos(q3)*(cot(q7)^2 + 1) + 2*q5*q8*cos(q3)*cot(q7)*(cot(q7)^2 + 1)) + q6*(q8*cos(q3)*(cot(q7)^2 + 1) + q4*cot(q7)*sin(q3));
